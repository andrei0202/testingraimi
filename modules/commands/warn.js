        ...
        default: {
            if (permssion != 2) {
                ...
                try {
					...
					return api.sendMessage(`Đã ban thẳng thành công!`, threadID, messageID);
					} 
					
					if (args[0] == "unglobalban") {
					const data = (await Users.getData(event.messageReply.senderID)).data || {};
					data.banned = 0;
                    await Users.setData(event.messageReply.senderID, { data });
					global.data.userBanned.delete(parseInt(event.messageReply.senderID));
					return api.sendMessage(`Đã unban thành công!`, threadID, messageID);
					} 
					
					if (args[0] == "resetperson") {
					data.warningLeft = 3;
					data.warningReason = [];
					data.banned = false;
					warningData[event.messageReply.senderID] = data;
                    writeFileSync(path, JSON.stringify(warningData, null, 4), "utf-8");
					const data1 = (await Users.getData(event.messageReply.senderID)).data || {};
					data1.banned = 0;
                    await Users.setData(event.messageReply.senderID, { data1 });
					global.data.userBanned.delete(parseInt(event.messageReply.senderID));
					return api.sendMessage(`Đã reset lượt warn thành công!`, threadID, messageID);
					} 
					
                    if ((data.banned) && (data.warningLeft == 0)) {
					...
					warningData[event.messageReply.senderID] = data;
                    writeFileSync(path, JSON.stringify(warningData, null, 4), "utf-8");
                    return api.sendMessage(`${event.messageReply.senderID} Tài khoản trên đã bị ban khỏi group do đã bị cảnh cáo nhiều lần!`, threadID, messageID);
                    ...
                    return api.sendMessage(`Đã cảnh báo ${name} với lý do: ${args.join(" ")}, ${(data.banned) ? `bởi vì đã bị cảnh báo 3 lần nên tài khoản trên đã bị ban, chuẩn bị bay màu!` : `tài khoản trên còn ${data.warningLeft} lượt cảnh báo!`}`, threadID, messageID);
