...
module.exports.run = ({ event, api, args, permssion }) => {
    ...
    switch (args[0]) {
        ...
        default: {
			if (number > 0 && number <= thisThread.listRule.length){
		var msg = "";
		msg += `${number}/ ${thisThread.listRule[number-1]}\n`;
		return api.sendMessage(`${msg}`, threadID, messageID);
	} 
            if (thisThread.listRule.length != 0) {
                var msg = "", index = 0;
                for (const item of thisThread.listRule) msg += `${index+=1}/ ${item}\n`;
                return api.sendMessage(`=== Luật của nhóm ===\n\n${msg} \n[Việc tuân thủ luật của nhóm sẽ giúp các bạn không bị bay màu :D !]`, threadID, messageID);
            }
            else return api.sendMessage(`[Rule] Có thể nhóm của bạn chưa thêm luật nào hoặc sai cú pháp !`, threadID, messageID);
        }
    }
