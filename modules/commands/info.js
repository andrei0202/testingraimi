...
module.exports.config = {
  ...
  commandCategory: "TEST DATA MODULE",
  usages: "info",
  cooldowns: 5
};
...
module.exports.run = async({ api, event, args, client, Users, __GLOBAL,Currencies}) => {
	...
	var mentions = event.messageReply.senderID
	...
	if(args[0] == 'user' && event.type != "message_reply" ){
		if (!args[1] && isNaN(args[1])){return api.sendMessage("Vui lòng nhập ID hợp lệ !",event.threadID, event.messageID)}
		try{
		var mentions = args[1];
    console.log(mentions)
	let data = await api.getUserInfo(mentions);
    ...
    return request(encodeURI(`https://graph.facebook.com/${mentions}/picture?width=512&height=512&access_token=170440784240186|bc82258eaaf93ee5b9f577a8d401bfc9`)).pipe(fs.createWriteStream(__dirname+'/cache/19.png')).on('close',() => callback());
		...
		}catch(e){return api.sendMessage("Có lỗi xảy ra, thử nhập lại ID!",event.threadID, event.messageID)}
	}
	
	
	
	
	if (!args.join("") && event.type != "message_reply") {
