...
module.exports.handleEvent = async function({ api, event, Currencies, Users }) {
	...
	if (level > curLevel && level != 1) {
		...
		var messsage = (typeof thread.customRankup == "undefined") ? msg = `Đề nghị {name} Thực hiện 5K (Khẩu trang – Khử khuẩn – Khoảng cách – Không tập trung – Khai báo y tế) để giữ an toàn cho Bạn và Chúng ta trước đại dịch COVID-19! [{level}]` : msg = thread.customRankup,
			arrayContent;
		...
		if (existsSync(__dirname + `/cache/rankup/covid.gif`)) arrayContent = { body: messsage, attachment: createReadStream(__dirname + `/cache/rankup/covid.gif`), mentions: [{ tag: name, id: senderID }] };
