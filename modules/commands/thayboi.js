...
module.exports.run = async function ({ api, event, args, Users, Threads, __GLOBAL, Currencies  }) {
    var data = ["Con bốc được lá 2 ♥ Để Thầy xem...Uhmm, một quân bài rất tốt. Nó báo hiệu rằng con sẽ được người yêu hoặc một người mến mộ ủng hộ và giúp đỡ. Thế nên là an tâm con nhá, có khó khăn gì thì người ấy cũng thông cảm và giúp con mà.",
  ...
  var money = data1.money
      if(money < 200) api.sendMessage("Bạn cần 200$ đô để xem bói ?",event.threadID,event.messageID)
          else {
			  Currencies.setData(event.senderID, options = {money: money - 200})
		  return api.sendMessage(`Xem Bói (- 200$)\n${data[Math.floor(Math.random() * data.length)]}`, event.threadID, event.messageID);
		  }
